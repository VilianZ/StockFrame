...
type Metric = { id: string; value: number | null; unit: string; formulaId?: string; status?: string; warnings?: string[]; evidenceIds?: string[] };
...
function formatCurrency(value: number, currency: string, maximumFractionDigits = 2) {
  return new Intl.NumberFormat("id-ID", { style: "currency", currency, maximumFractionDigits }).format(value);
}

function formatMetric(value: number | null, unit: string, id?: string, currency = "USD") {
  ...
  if (unit === "currency_per_share") return formatCurrency(value, currency);
  if (unit === "currency" && Math.abs(value) >= 1_000_000_000) return `${formatCurrency(value / 1_000_000_000, currency, 1)} Miliar`;
  if (unit === "currency") return formatCurrency(value, currency, 1);
...
export default function HomePage() {
  ...
  const metric = (id: string, fallback = "—") => {
    ...
    if (item) return formatMetric(item.value, item.unit, canonicalId, viewData?.instrument.currency);
...
function SummaryView({ data, metric, activeProfile, profile, qualitySegments, onTab }: { data: AppData; metric: (id: string, fallback?: string) => string; activeProfile: ProfileId; profile: Profile; qualitySegments: boolean[]; onTab: (tab: TabId) => void }) {
  ...
  const qualityFlags = new Set(data.quality.flags);
  return <div className="tab-content summary-content"><section className="summary-hero"><div className="hero-copy"><span className="eyebrow">KESIMPULAN AI · {PROFILE_LABELS[activeProfile].toUpperCase()}</span><h2>{claimText(data.report.summary)}</h2><p>{claimText(profile.thesis)}</p><div className="hero-metrics"><Stat label="ROE" value={metric("roe", "—")} note="TTM" /><Stat label="Margin bersih" value={metric("net_margin", "—")} note="TTM" /><Stat label="P/E" value={metric("pe_ratio", "—")} note="TTM" /></div></div><div className="verdict"><span className="eyebrow">VERDIK</span><strong>{PROFILE_LABELS[activeProfile].toUpperCase()}</strong><span className="eyebrow">KEYAKINAN</span><b>{Math.round(profile.confidence * 100)}%</b></div></section><div className="summary-grid"><section className="panel price-panel"><SectionTitle title="Riwayat harga" action="1 tahun" /><Chart prices={data.snapshot.prices} currency={data.instrument.currency} /></section><section className="panel quality-panel"><SectionTitle title="Kualitas data" /><div className="quality-score"><strong>{Math.round(data.quality.score)}%</strong><div className="quality-segments">{qualitySegments.map((filled, index) => <i className={filled ? "filled" : ""} key={index} />)}</div></div><div className="quality-list"><div><span>Coverage finansial</span><b>{qualityFlags.has("missing_financial_data") ? "Perlu perhatian" : "Memadai"}</b></div><div><span>Keterkinian snapshot</span><b>{qualityFlags.has("stale_data") ? "Perlu perhatian" : "Memadai"}</b></div><div><span>Konsistensi data</span><b>{qualityFlags.has("currency_mismatch") || qualityFlags.has("inconsistent_statements") ? "Perlu perhatian" : "Terjaga"}</b></div></div><div className="source-row"><Icon name="database" size={21} /><div><strong>Business Quant</strong><small>Laporan keuangan & estimasi</small></div><span className="active-dot">Aktif</span></div></section><section className="panel metrics-panel"><SectionTitle title="Metrik utama" action="TTM" /><div className="metric-strip"><Stat label="Pendapatan" value={metric("revenue_ttm")} /><Stat label="Laba bersih" value={metric("net_income_ttm")} /><Stat label="ROA" value={metric("roa")} /><Stat label="ROE" value={metric("roe")} /><Stat label="Debt to equity" value={metric("debt_to_equity")} /><Stat label="P/E" value={metric("pe_ratio")} /></div></section><section className="panel actions-panel"><SectionTitle title="Aksi korporasi terbaru" action="Lihat semua" /><ActionList events={events.slice(0, 2)} /><button className="text-button" onClick={() => onTab("actions")}>Lihat semua aksi korporasi <Icon name="arrow" size={14} /></button></section><section className="panel why-panel"><SectionTitle title="Mengapa kesimpulan ini?" action="Bukti" /><ol className="reason-list">{data.report.strengths.slice(0, 3).map((claim, index) => <li key={index}><span>{index + 1}</span><p>{claimText(claim)}</p><small>{claim.metricIds?.join(" · ") || "Evidence laporan"}</small></li>)}</ol><button className="text-button" onClick={() => onTab("evidence")}>Lihat semua bukti <Icon name="arrow" size={14} /></button></section></div><Disclaimer text={data.report.disclaimer} /></div>;
}

function HistoryView({ data }: { data: AppData }) {
  const sortedPrices = sortPrices(data.snapshot.prices);
  const first = sortedPrices[0]?.close ?? null;
  const last = sortedPrices.at(-1)?.close ?? null;
  const change = first && last ? ((last - first) / first) * 100 : 0;
  const currency = data.instrument.currency;
  const price = (value: number | null) => value === null ? "—" : formatCurrency(value, currency);
  const lowest = sortedPrices.length ? Math.min(...sortedPrices.map((point) => point.close)) : null;
  const highest = sortedPrices.length ? Math.max(...sortedPrices.map((point) => point.close)) : null;
  const median = sortedPrices.length ? [...sortedPrices.map((point) => point.close)].sort((a, b) => a - b)[Math.floor(sortedPrices.length / 2)] ?? null : null;
  return <div className="tab-content history-content"><div className="tab-intro"><div><span className="eyebrow">PRICE LEDGER · 12 BULAN</span><h2>Pergerakan harga dalam konteks.</h2><p>Baca ritme harga bersama rentang waktu dan titik data yang menjadi dasar chart.</p></div><div className="intro-stat"><span>Perubahan periode</span><strong className={change >= 0 ? "positive" : "negative"}>{change >= 0 ? "+" : ""}{change.toFixed(1)}%</strong></div></div><section className="panel large-chart-panel"><SectionTitle title="Riwayat harga penutupan" action={`${data.snapshot.prices.length} titik data`} /><Chart prices={data.snapshot.prices} large currency={currency} /><div className="chart-foot"><span><i className="legend-line" />Harga penutupan</span><span>Terendah {price(lowest)}</span><span>Tertinggi {price(highest)}</span></div></section><div className="two-column"><section className="panel"><SectionTitle title="Snapshot harga" /><div className="big-number">{price(data.snapshot.price)}<small>Harga terakhir · {formatDate(data.snapshot.asOf)}</small></div><div className="mini-stats"><Stat label="Awal periode" value={price(first)} /><Stat label="Median" value={price(median)} /><Stat label="Akhir periode" value={price(last)} /></div></section><section className="panel"><SectionTitle title="Cara membaca" /><div className="note-list"><div><span>01</span><p>Tren harga bukan pengganti analisis fundamental.</p></div><div><span>02</span><p>Perubahan periode dihitung dari titik yang tersedia.</p></div><div><span>03</span><p>Gunakan chart sebagai konteks volatilitas.</p></div></div></section></div><Disclaimer text="Harga historis bukan jaminan hasil di masa depan." /></div>;
}
...
/* function EvidenceView({ data }: { data: AppData }) {
  const claims: Claim[] = [...data.report.strengths.slice(0, 3), ...data.report.risks.slice(0, 2), ...(data.report.corporateActionClaims ?? []).map((item) => ({ text: item.claim, evidenceId: item.evidenceId }))];
...
function EvidenceView({ data }: { data: AppData }) {
  const claims: Claim[] = [...data.report.strengths.slice(0, 3), ...data.report.risks.slice(0, 2), ...(data.report.corporateActionClaims ?? []).map((item) => ({ text: item.claim, evidenceId: item.evidenceId }))];
  return <div className="tab-content evidence-content"><div className="tab-intro"><div><span className="eyebrow">EVIDENCE LEDGER · TRACEABILITY</span><h2>Kesimpulan yang meninggalkan jejak.</h2><p>Telusuri asal data, batas interpretasi, dan alasan mengapa kualitas analisis mendapat skornya.</p></div><div className="intro-stamp quality-stamp"><strong>{Math.round(data.quality.score)}%</strong><span>kualitas<br />data</span></div></div><div className="evidence-grid"><section className="panel claim-ledger"><SectionTitle title="Peta bukti kesimpulan" action={`${claims.length} klaim`} />{claims.map((claim, index) => <div className="claim-row" key={`${claim.evidenceId ?? "claim"}-${index}`}><span className={index < 3 ? "claim-number lime" : "claim-number"}>{String(index + 1).padStart(2, "0")}</span><div><p>{claimText(claim)}</p><small>{claim.evidenceId ? `Evidence ${claim.evidenceId}` : claim.metricIds?.join(" · ") || "Konteks laporan"}</small></div><span className="source-badge">Business Quant <Icon name="external" size={12} /></span></div>)}</section><section className="panel source-ledger"><SectionTitle title="Sumber utama" /><div className="source-card"><Icon name="database" size={22} /><div><strong>Business Quant</strong><p>Harga, laporan keuangan, metrik, dan aksi korporasi.</p></div><span className="active-dot">Aktif</span></div><div className="source-card"><Icon name="brain" size={22} /><div><strong>Gemini</strong><p>Interpretasi profil risiko dengan kontrak output terstruktur.</p></div><span className={data.quality.aiEligible ? "active-dot" : "muted-dot"}>{data.quality.aiEligible ? "Aktif" : "Terbatas"}</span></div><div className="quality-breakdown">{data.quality.notes.slice(0, 3).map((note) => <span key={note}>{note}</span>)}{data.quality.flags.length ? <span>Flag: <b>{data.quality.flags.join(" · ")}</b></span> : null}</div></section></div><section className="panel limitations-panel"><SectionTitle title="Bukti batasan" action="Baca sebelum memakai" /><div className="limitation-list">{data.report.limitations.map((limitation, index) => <div key={index}><Icon name="alert" size={17} /><p>{limitation}</p></div>)}</div></section></div>;
}

function ActionList({ events }: { events: CorporateAction[] }) { return <div className="action-list">{events.length ? events.map((event, index) => <div className="action-item" key={`${event.date}-${index}`}><span className="action-icon"><Icon name={event.kind === "dividend" ? "spark" : "bars"} size={17} /></span><div><strong>{event.rawAction}</strong><small>{formatDate(event.date)}{event.value !== null ? ` · ${event.value}` : ""}</small></div><p>{event.notes ?? "Aksi korporasi tercatat pada sumber utama."}{event.relatedTicker ? ` · Relasi ${event.relatedTicker}` : ""}</p><Icon name="arrow" size={15} /></div>) : <p className="muted-copy">Belum ada aksi korporasi.</p>}</div>; }
